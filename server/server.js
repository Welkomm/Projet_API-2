...
const stripe = require('stripe')('sk_test_51Q5nEq06yVlPv7G4qJuHQKerz7pacM8BF5Slr6yOfPVAF7Ih251RbDCY63vjd5itvA0xjaZSJN0ssQdJjVmkhfFz00itQ5g39H'); // Remplacez par votre clé secrète de test
...
app.post('/create-checkout-session', async (req, res) => {
    try {
        ...
        // Vérification des données du panier (à adapter selon votre structure de données)
        ...
        const lineItems = cartItems.map(item => ({
            price_data: {
                ...
                product_data: {
                    name: item.title,
                    // Vous pouvez ajouter d'autres informations sur le produit ici (description, image, etc.)
                },
                unit_amount: Math.round(item.price * 100), // Convertir le prix en centimes (attention aux arrondis)
            },
        ...
        const session = await stripe.checkout.sessions.create({
            ...
            mode: 'payment',
            success_url: `http://localhost:3001/order-success/${username}`, // Redirection après un paiement réussi (avec username en paramètre)
            cancel_url: 'http://localhost:3001/order-canceled', // Redirection après un paiement annulé
        });
